...
范围（v1）：展示连接状态与安全形式的 API origin、重试健康检查、管理本地偏好、恢复默认值、展示版本信息。v1 的版本信息仅指前端自身版本（构建期注入的公开版本号）；API 契约未提供后端版本端点，后端版本一律显示为「不可用」，禁止编造。
...
- 普通用户不得修改 API Base URL：API Base URL 由构建时环境变量注入，禁止在浏览器运行时改写。
...
- 前端环境变量是公开配置，绝不包含 secret；部署配置只由环境变量与后端安全配置控制。
